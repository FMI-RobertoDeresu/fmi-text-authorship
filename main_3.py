...
def svm_with_rank_kernel(dataset):
    ...
    train_data = (list(map(lambda x: feature_extraction.ranking_distance(x[2]), train_dataset_values)),
                  list(map(lambda x: federalist_papers_path_authors[x[1]], train_dataset_values)))

    predict_data = (list(map(lambda x: feature_extraction.ranking_distance(x[2]), eval_dataset_values)),)
    ...
    names = svm.get_variable_names()
    for i in names:
        print(i)
        print(svm.get_variable_value(i))
