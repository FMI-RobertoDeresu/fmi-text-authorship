...
tf.flags.DEFINE_float('train_kernel_stddev', 0.001, 'RandomFourierFeatureMapper stddev.')
tf.flags.DEFINE_integer('train_kernel_output_dim', 784, 'RandomFourierFeatureMapper output_dim.')
...
def run_kernel_model(train_data, eval_data):
        ...
        # img = train_data[0][0].astype(np.float32)
        # plt.imshow(img, shape=(28, 28))
        # plt.show()
        #
        # input_img_tensor = tf.Variable(img.reshape(1, 784,))
        # output_img_tensor = kernel_mapper.map(input_img_tensor)
        #
        # with tf.Session() as sess:
        #     sess.run(input_img_tensor.initializer)
        #     img = sess.run(output_img_tensor)[0].reshape(28,28)
        #
        #     plt.imshow(img, shape=(28, 28))
        #     plt.show()
        #
